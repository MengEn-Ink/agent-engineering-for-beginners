...
6. **关键源码入口**：3–13 个固定 commit 文件链接，写清文件职责，并显示该文件在本页使用的全部符号；
...
- **唯一默认链：** 固定 `Process.sequential + Task.async_execution=false + Agent.planning=false`：`Crew.kickoff` → `begin_execution` → `prepare_kickoff` / `setup_agents` / `Agent.create_agent_executor` → kickoff 内判断 `self.process == Process.sequential` → `Crew._run_sequential_process` / `_execute_tasks` → `prepare_task_execution` → `Task.execute_sync` / `_execute_core` → `Agent.execute_task` → 默认 `experimental.AgentExecutor.invoke` → `AgentFinish.output` → `Agent._finalize_task_execution` → `TaskOutput` → `Crew._create_crew_output` → `end_execution`。
- **关键入口：**
...
- 六个核心页各包含模板 13 个部分、一个主调用链、3–13 个固定源码入口和至少一个失败边界；全目录精确包含 66 个文件级 entrypoint。OpenHands 页面固定 9 个入口，评测页固定 12 个入口（SWE-bench 4 个、τ²-bench 8 个），Dify 固定 13 个，CrewAI 固定 11 个；Dify 与 CrewAI 多轨图中的每条可视化 track 均不得超过 13 个节点。
